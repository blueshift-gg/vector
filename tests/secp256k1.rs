...
use vector_core::{
    ...
    find_vector_pda, secp256k1_compressed_pubkey, sign_advance_instruction_secp256k1_ecdsa,
    SECP256K1,
};
...
fn advance_empty() {
    ...
    let vector_account = build_vector_account(
        ...
        &SECP256K1,
        bump,
        mollusk.sysvars.rent.minimum_balance(SECP256K1.account_len()),
        &identity,
    );
...
fn advance_round_trips_spl_mint_authority() {
    let key = signing_key();
    let identity = secp256k1_compressed_pubkey(&key);
    run_round_trip_spl(&SECP256K1, &identity, &identity, |nonce, sub, pre, post| {
        sign_advance_instruction_secp256k1_ecdsa(&key, nonce, sub, pre, post)
    });
}
...
fn close_via_advance() {
    ...
    let vector_lamports = mollusk.sysvars.rent.minimum_balance(SECP256K1.account_len());
    let (vector, bump) = find_vector_pda(&SECP256K1, &identity);
    ...
    let advance_ix = sign_advance_instruction_secp256k1_ecdsa(&key, &NONCE, &[close_sub], &[], &[]);
