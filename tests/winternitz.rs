...
use vector_core::{
    ...
    create_initialize_winternitz, create_passthrough_instruction, find_vector_pda, WINTERNITZ,
};
...
fn initialize_and_client_encoding() {
    ...
    let (vector, bump) = find_vector_pda(&WINTERNITZ, &public_key);
    ...
    let close = create_close_subinstruction(&WINTERNITZ, &public_key, &receiver);
    let passthrough = create_passthrough_instruction(&WINTERNITZ, &public_key, &[close]);
    assert_eq!(
        advance_vector_digest(&WINTERNITZ, &NONCE, &public_key, &[], &[passthrough]),
        [
            81, 44, 155, 242, 25, 64, 39, 109, 129, 125, 197, 57, 95, 46, 66, 237, 3, 234, 200, 33,
            148, 221, 235, 122, 208, 136, 39, 95, 18, 2, 113, 238
        ],
    );
...
fn signature_binds_close_and_rejects_replay() {
    ...
    let lamports = mollusk
        ...
        + 5_000_000;
    let account = build_vector_account(NONCE, &WINTERNITZ, bump, lamports, &public_key);
    ...
    let advance = create_advance_instruction(&WINTERNITZ, &public_key, &signature);
    let expected = expected_advanced_data(digest, &WINTERNITZ, bump, &public_key);

    let other_close = create_close_subinstruction(&WINTERNITZ, &public_key, &other);
    let changed_action = create_passthrough_instruction(&WINTERNITZ, &public_key, &[other_close]);
    ...
    for (bytes, state, action, error) in [
        ...
        let invalid = create_advance_instruction(&WINTERNITZ, &public_key, bytes);
